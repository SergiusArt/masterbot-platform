"""Bablo activity settings handlers."""
...
class BabloActivitySettingsState(StatesGroup):
    """States for bablo activity settings input."""

    waiting_for_window = State()
    waiting_for_threshold = State()


@router.message(MenuState.bablo, F.text == MENU_ACTIVITY)
async def activity_menu(message: Message, state: FSMContext) -> None:
    ...
    await message.answer(
        ...
        "произойдёт указанное количество сигналов.",
        reply_markup=get_activity_menu_keyboard(window, threshold),
    )


@router.message(MenuState.bablo_activity, F.text.startswith("⏱"))
async def change_activity_window(message: Message, state: FSMContext) -> None:
    ...
    Args:
        ...
        state: FSM context
    """
    await state.set_state(BabloActivitySettingsState.waiting_for_window)
    await message.answer(
        "⏱ <b>Окно активности</b>\n\n"
...
async def change_activity_threshold(message: Message, state: FSMContext) -> None:
    ...
    Args:
        ...
        state: FSM context
    """
    await state.set_state(BabloActivitySettingsState.waiting_for_threshold)
    await message.answer(
        "📊 <b>Порог активности</b>\n\n"
        "Введите количество сигналов (от 1 до 100):",
        reply_markup=get_back_keyboard(),
    )


@router.message(BabloActivitySettingsState.waiting_for_window, F.text == MENU_BACK)
...
@router.message(BabloActivitySettingsState.waiting_for_window)
...
@router.message(BabloActivitySettingsState.waiting_for_threshold)
